...
Reassign genes, only 1 gene per snp, remove the genes in LD
518610 snps_in_ld.txt
3799647 plink.withGene.bim

20261 gene_names_coordinates_plink_final_noLD.txt
'''
...
no_lD_file="/gpfs/group/stsi/data/projects/wellderly/GenomeComb/pathway_analysis/snps_in_ld.txt"

#Index the LD file
LD_file={}
with open(no_lD_file) as f:
    for line in f:
        items = line.strip().split("\t")
        LD_file[items[0]] = "Y"

print "LD dictionary size " + str(len(LD_file))
...
overlapping_gene_file=open("/gpfs/home/gerikson/wellderly/resources/overlapping_genes.txt","a")

outfilename="/gpfs/home/gerikson/wellderly/resources/gene_names_coordinates_plink_final_noLD.txt"
...
for line in infile:
    ...
    if tp_line[0] != chrom:
        for g in gene_dict:
            ...
            gene_indexes = ""
            for i in coord:
                gene_indexes = gene_indexes + "\t" + str(i)
            outfile.write(chrom + "\t" + g +"\t"+ gene_indexes+ "\n")
    ...
    #IF this snp found in the LD file

    genes = tp_line[1].split("_")
    final_snp_coord = genes[0]
    try:
        ld = LD_file[final_snp_coord]
        ...
        if len(gen) == 1:  
            ...
            for g in gen:
                ...
                if snppos >= begin and snppos <= end:
                    ...
                    minimum=min(abs(snppos-end),abs(begin-snppos))
                    min_distance.append(minimum) 
                    gene_with_coords.append(g)

            if snp_inside_gene == 0:
                min_dist = 10000000
                gene = ""
                for index, i in enumerate(min_distance):
                    if i < min_dist:
                        min_dist = i
                        gene = gene_with_coords[index]
                        final_gene = gene

                #print "closest gene: " + gene
                #print "distance from gene " + str(min_dist)


                #Verify if this gene is already present
                #If present update the end coordinate, start coordinate doesn't need updating
                try:
                    coords = gene_dict[gene]
                    coords.append(counter)
